...
def _get_index_group(data: DataFrame, index: DataFrame, group: Any) -> DataFrame | Series:
    """Return an index column robustly across pandas versions."""
    if group in index.columns:
        return index[group]
    reset_index = data.reset_index()
    if group in reset_index.columns:
        return reset_index[group]
    return index[group]
    ...
    def fit(self, data: DataFrame, /) -> None:
        ...
        if self.index_encoders is None:
            ...
            for group, encoder in self.index_encoders.items():
                group_data = _get_index_group(data, index, group)
                encoder.fit(group_data)
                encoded = encoder.encode(group_data)
