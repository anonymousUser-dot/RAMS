...
class Loss(nn.Module):
    """MSE with paired tail magnitude and smooth harm-frequency control."""
    ...
    def __init__(self, configs: ExpConfigs):
        ...
        self.ntr_temperature = max(float(getattr(configs, "apn_paired_ntr_temperature", 1e-3)), 1e-6)
    ...
    def forward(self, pred, true, mask=None, base_pred=None, **kwargs):
        ...
        if self.ntr_weight > 0.0:
            loss = loss + self.ntr_weight * torch.sigmoid(excess / self.ntr_temperature).mean()
