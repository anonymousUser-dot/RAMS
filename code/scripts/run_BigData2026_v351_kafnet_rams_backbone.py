#!/usr/bin/env python
"""v351 KAFNet + RAMS mechanism-state residual confirmation.

This runner complements the RAMS-tPatch paper experiments with a second
architecture.  It keeps the same backbone-preserving residual protocol, but
uses KAFNet as the frozen public backbone:

  1. train or reuse matched KAFNet checkpoints;
  2. train MCORWrapper residual branches on the frozen KAFNet state;
  3. summarize paired MSE/MAE gains against the matched KAFNet base.

The default is intentionally compact: the wearable suite plus HumanActivity,
three seeds, and the learned gate variant.  Additional variants can be enabled
for ablation with --variants local,integral,mix,gate,safe_gate.
"""
...
DATASETS: dict[str, DatasetSpec] = {
    "HumanActivity": DatasetSpec("storage/datasets/HumanActivity", 3000, 300, 300, 12, 16, 16, 0.0, 300, 8),
    "MHEALTH": DatasetSpec("storage/datasets/MHEALTH", 250, 50, 25, 23, 16, 32, 0.05, 50, 8),
    "OPPORTUNITY": DatasetSpec("storage/datasets/OPPORTUNITY", 300, 60, 30, 64, 24, 32, 0.05, 60, 8),
    "PAMAP2": DatasetSpec("storage/datasets/PAMAP2", 500, 100, 50, 37, 24, 32, 0.05, 50, 8),
}
...
def latest_metric(root: Path, dataset: str, model_name: str, model_id: str) -> dict[str, float] | None:
    paths = sorted((root / dataset / model_name / model_id).glob("*/*/iter*/eval_*/metric.json"))
    if not paths:
        return None
...
def base_id(dataset: str, seed: int, epochs: int) -> str:
    return f"local_baselines_{dataset}_kafnet_e{epochs}_s{seed}"


def wrapper_id(prefix: str, dataset: str, variant: str, seed: int, residual_epochs: int) -> str:
    return f"{prefix}_{dataset}_KAFNet_rams_{variant}_e{residual_epochs}_s{seed}"


def find_base(args: argparse.Namespace, dataset: str, seed: int) -> tuple[Path | None, dict[str, float] | None, Path | None]:
    mid = base_id(dataset, seed, args.base_epochs)
    for root_value in csv_list(args.base_roots):
        root = (REPO / root_value).resolve()
        ckpt = latest_checkpoint(root, dataset, "KAFNet", mid)
        metric = latest_metric(root, dataset, "KAFNet", mid)
...
def ensure_base(args: argparse.Namespace, dataset: str, seed: int, root: Path) -> int:
    ...
    if ckpt is not None and metric is not None:
        print(f"[base-ready] {dataset} KAFNet seed={seed} {metric}", flush=True)
        return 0
    return run([
        sys.executable,
    ...
    ], root / "_logs" / f"ensure_kafnet_{dataset}_s{seed}.log", bool(args.dry_run))
...
def summarize(args: argparse.Namespace, root: Path, datasets: list[str], seeds: list[int], variants: list[str]) -> Path:
    ...
    for dataset in datasets:
        for seed in seeds:
            ...
            base_mid = base_id(dataset, seed, args.base_epochs)
            rows.append({
                "dataset": dataset,
                "seed": seed,
                "model": "KAFNet",
    ...
    detail = out_dir / "v351_kafnet_rams_detail.csv"
    ...
    agg = out_dir / "v351_kafnet_rams_agg.csv"
...
def main() -> int:
    ...
    parser.add_argument("--root", default="storage/results_BigData2026_v351_kafnet_rams")
    parser.add_argument("--base_roots", default="storage/results_BigData2026_v351_kafnet_rams,storage/results_BigData2026_v322_public_baselines_e4,storage/results_BigData2026_v313_public_baselines_e4")
    parser.add_argument("--prefix", default="v351")
    ...
    parser.add_argument("--variants", default="gate")
