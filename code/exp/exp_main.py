...
class Exp_Main(Exp_Basic):
    ...
    def _move_batch_to_device(self, batch: dict[str, Tensor]) -> dict[str, Tensor]:
        if self.configs.use_multi_gpu:
            return batch
        non_blocking = bool(
            self.configs.use_gpu
            and self.configs.pin_memory
            and self.configs.non_blocking_transfer
        )
        return {
            k: v.to(self.device, non_blocking=non_blocking) if isinstance(v, Tensor) else v
    ...
    def _autocast_context(self):
        if (
            self.configs.use_gpu
            and self.configs.amp_dtype == "bf16"
    ...
    def _progress_disabled(self) -> bool:
        return bool(self.configs.disable_tqdm) or not accelerator.is_main_process
    ...
    def _add_selector_diag(self, diag_acc: dict, key: str, value) -> None:
        diag_acc.setdefault(key, []).append(np.asarray(value, dtype=float))

    def _mean_over_batch_vars(self, tensor: Tensor):
        if tensor is None:
            return None
        ...
        return tensor.mean(dim=reduce_dims).cpu().numpy()

    def _collect_selector_diagnostics(self, model: Module, diag_acc: dict) -> None:
        wrapped_model = getattr(model, "module", model)
        selector_model = getattr(wrapped_model, "model", None)
        if selector_model is None:
            return

        gates = getattr(selector_model, "latest_selector_gates", None)
        if gates is None:
            return

        branch_gates = getattr(selector_model, "latest_selector_branch_gates", None)
        safety_gate = getattr(selector_model, "latest_selector_safety_gate", None)
        selector_stats = getattr(selector_model, "latest_selector_stats", None)
        selector_uncertainty = getattr(selector_model, "latest_selector_uncertainty", None)
        residual_norms = getattr(selector_model, "latest_selector_residual_norms", None)
        selector_price = getattr(selector_model, "latest_selector_price", None)
        selector_safe_scores = getattr(selector_model, "latest_selector_safe_scores", None)
        selector_price_components = getattr(selector_model, "latest_selector_price_components", None)

        self._add_selector_diag(diag_acc, "effective_gate_mean", self._mean_over_batch_vars(gates))
        self._add_selector_diag(diag_acc, "effective_gate_active_0_01", self._mean_over_batch_vars((gates > 0.01).float()))
        if branch_gates is not None:
            self._add_selector_diag(diag_acc, "raw_branch_gate_mean", self._mean_over_batch_vars(branch_gates))
        if safety_gate is not None:
            self._add_selector_diag(diag_acc, "safety_gate_mean", self._mean_over_batch_vars(safety_gate))
            self._add_selector_diag(diag_acc, "safety_gate_active_0_01", self._mean_over_batch_vars((safety_gate > 0.01).float()))
        if selector_stats is not None:
            self._add_selector_diag(diag_acc, "structural_stats_mean", self._mean_over_batch_vars(selector_stats))
        if selector_uncertainty is not None:
            self._add_selector_diag(diag_acc, "uncertainty_score_mean", self._mean_over_batch_vars(selector_uncertainty))
        if selector_price is not None:
            self._add_selector_diag(diag_acc, "operator_price_mean", self._mean_over_batch_vars(selector_price))
        if selector_safe_scores is not None:
            self._add_selector_diag(diag_acc, "operator_safe_score_mean", self._mean_over_batch_vars(selector_safe_scores))
        if selector_price_components is not None:
            self._add_selector_diag(diag_acc, "operator_price_components_mean", self._mean_over_batch_vars(selector_price_components))
        if isinstance(residual_norms, dict):
            for name, value in residual_norms.items():
                if value is not None:
                    self._add_selector_diag(diag_acc, f"residual_energy_{name}", value.detach().float().cpu().numpy())

    def _summarize_selector_diagnostics(self, diag_acc: dict):
        if not diag_acc:
            return None
        summary = {
            "num_batches": max(len(values) for values in diag_acc.values()),
            "gate_names": ["spectral", "trend", "covariance", "decoder"],
            "stat_names": ["density", "missing", "missing_heterogeneity", "irregularity", "spectral_entropy", "covariance_energy", "patch_smoothness", "quadrature_uncertainty", "recency", "observed_span"],
            "operator_price_component_names": ["interp", "quadrature", "dependency", "missing_sensitivity", "smooth"]
        }
        for key, values in diag_acc.items():
            try:
                mean_value = np.mean(np.stack(values), axis=0)
            except ValueError:
                logger.warning(f"Skipping selector diagnostic '{key}' because recorded shapes are inconsistent")
                continue
            if np.ndim(mean_value) == 0:
                summary[key] = float(mean_value)
            else:
                summary[key] = mean_value.astype(float).tolist()
        return summary
    ...
    def vali(
        ...
        model_train.eval()
        with torch.no_grad():
            with tqdm(total=len(vali_loader), leave=False, desc="Validating", disable=self._progress_disabled()) as it:
    ...
    def train(self) -> None:
        ...
        if self.configs.pretrained_checkpoint_root_path:
            pretrained_root = Path(self.configs.pretrained_checkpoint_root_path)
            pretrained_name = self.configs.pretrained_checkpoint_file_name or "pytorch_model.bin"
            pretrained_file = pretrained_root / pretrained_name
            if pretrained_file.exists():
                try:
                    original_state_dict = self._get_state_dict(pretrained_file)
                    load_result = model_train.load_state_dict(original_state_dict, strict=False)
                    logger.info(
                        "Warm-started training from %s with %d missing and %d unexpected keys.",
                        pretrained_file,
                    ...
                    if load_result.missing_keys or load_result.unexpected_keys:
                        logger.warning(
                            "Warm-start non-strict load details: missing_keys=%s unexpected_keys=%s",
                            load_result.missing_keys,
                            load_result.unexpected_keys,
                        )
                except Exception as e:
                    logger.exception(f"{e}", stack_info=True)
                    logger.exception(f"Failed to warm-start from {pretrained_file}. Continuing from random init.")
            else:
                logger.warning("pretrained checkpoint file does not exist: %s", pretrained_file)
        ...
        for train_stage in range(1, self.configs.n_train_stages + 1):
            ...
            for epoch in tqdm(range(self.configs.train_epochs), desc="Epochs", disable=self._progress_disabled()):
                train_loss = []
                model_train.train()
                with tqdm(total=len(train_loader), leave=False, desc="Training", disable=self._progress_disabled()) as it:
                    batch: dict[str, Tensor] # type hints
    ...
    def test(self) -> None:
        ...
        if self.configs.test_dataset_statistics:
            ...
            for batch in tqdm(data_loader, disable=self._progress_disabled()):
        ...
        flag = "test_all" if self.configs.test_all else self.configs.test_split
        if flag != "train" and getattr(self.configs, "seq_len_max_irr", None) is None:
            # Some irregular datasets infer padded lengths from the training split.
            # Direct validation/test-only audits need the same initialization.
        ...
        if not self.configs.use_multi_gpu:
            ...
            with torch.no_grad():
                batch: dict[str, Tensor] # type hints
                for i, batch in tqdm(enumerate(test_loader), total=len(test_loader), leave=False, desc="Testing", disable=self._progress_disabled()):
                    # warn if the size does not match
                    ...
                    self._collect_selector_diagnostics(model_test, selector_diag_acc)
                    ...
                    for tensor_name in input_tensor_names:
                        if tensor_name in batch_all.keys():
                            array_dict[tensor_name].append(batch_all[tensor_name].detach().float().cpu().numpy())
                    for tensor_name in output_tensor_names:
                        if tensor_name in outputs_all.keys():
                            array_dict[tensor_name].append(outputs_all[tensor_name].detach().float().cpu().numpy())
            ...
            selector_diag_summary = self._summarize_selector_diagnostics(selector_diag_acc)
            if selector_diag_summary is not None and accelerator.is_main_process:
                logger.info("Selector diagnostics:\n%s", json.dumps(selector_diag_summary, indent=4))
                with open(folder_path / "selector_diagnostics.json", "w") as f:
                    json.dump(selector_diag_summary, f, indent=2)
